...
- Termux shortcuts for laptop-hosted web UIs should not hard-code a DHCP address. The `send-ifconfig.service` user unit on the laptop runs `utilities/bin/send-ifconfig`, which publishes `~/laptop-lan.env` on `phone2`; `/etc/NetworkManager/dispatcher.d/90-send-ifconfig` starts that service after usable network changes, and shortcuts such as `overseerr-gui` and `present-window-gui` should read the published file before calling `termux-open-url`.
...
Global Vim-style text registers use the `vim-registers` cachectl namespace. Neovim exports named yanks through `nvim/dot-config/nvim/lua/serranomorante/vim_register_clipboard.lua`, `utilities/bin/vim-register-clipboard` reads and pastes cached register values, and the keyd path is owned by `playbooks/roles/10-system-tools/templates/keyd-default.conf` plus `peripherals/bin/keyd-observer`. Keep the system-wide paste gesture behind the existing keyd Tab modifier layer so normal quote typing remains untouched.
...
In Neovim buffers, executable shell fences can be run with `<leader>mr` wherever the current buffer lines contain a `sh`/`bash`/`shell` fence, including file-backed Markdown, scratch, `nofile`, and terminal buffers in Terminal-Normal mode. In Visual mode, the same keymap runs the selected lines directly as a shell script, so a multiline selection does not need to include a fenced `sh` block. The keymap launches the current fence or selection through Overseer and opens the task output in a regular window, so generated notes can keep commands as snippets instead of storing volatile log output. Shell-fence tasks should keep their generated script until task dispose so Overseer restart can rerun the same fence after completion. Shell fences that invoke `ansible` or `ansible-playbook` with `-K`/`--ask-become-pass` should prompt through a concealed Overseer form when `vim.g.pass` is empty, cache that value in `vim.g.pass`, schedule task startup after form submit so Overseer can finish its own window cleanup, and attach the shared Ansible become-password sender component so later Ansible fences and the dedicated Ansible picker can reuse the same in-memory password. Keep this path covered by `tests/nvim/overseer_shell_fences.sh`; shell-fence tasks should include Overseer's `on_complete_dispose` with `require_view` for success and failure statuses so completed fence runs do not keep polluting task history after their output has been shown, and their cwd resolver must fall back to `getcwd()` when the source buffer has no local file path.
...
- `utilities/bin/setup-displays.sh` owns the local display layout policy and manual laptop/external monitor toggle. XRandR applies the selected laptop-only or external-only layout. `setup-displays.sh --toggle` switches between those layouts from the `Tab+Shift+M` keyd signal handled by `peripherals/bin/keyd-observer`. The return to the laptop panel should enable the internal output as primary at `0x0` and disable the external output in one RandR transaction, then force DPMS on and refresh the internal mode even when DRM reports `enabled`/`On`, because this hybrid stack can leave the panel visually black while XRandR and DRM both report an active output. The root cause of the recurring black-but-lit panel is the X compositor (`picom`, run as the `compositor.service` user unit), not the GPU scanout: after an XRandR geometry change picom can keep a stale black overlay over the new layout, which no mode or DPMS refresh clears. So after either toggle direction `setup-displays.sh` restarts the compositor user unit (only when it is already active, to stay off the boot/`auto` path); the unit name is overridable via `SETUP_DISPLAYS_COMPOSITOR_SERVICE`. Keep this interactive path free of desktop notifications so Plasma does not enter the display-toggle hot path. Avoid always-on polling watchers for monitor power changes; manual keyd-triggered layout changes are preferred because routine display probing can be expensive on this workstation.
- `utilities/bin/present-window` owns the selected-window mirror workflow for already-running X11 apps and browser tab sharing. It serves noVNC on `0.0.0.0:14510`, keeps a runtime set of selected window IDs, and restarts a localhost-only `x11vnc` backend on the selected focused window so the web client follows focus without exposing the raw VNC port to the LAN. `Tab+Shift+W` is handled by `peripherals/bin/keyd-observer` and only toggles the focused window in that selected set; `Tab+Shift+P` opens the noVNC tab through `present-window open`.
...
- `utilities/bin/x11-monitor-geometry` is the shared helper for monitor-relative runtime geometry. Use it when wrappers need the current monitor's `WxH`, XRandR-style geometry, or X11 capture input rather than hard-coding a laptop-panel resolution.
