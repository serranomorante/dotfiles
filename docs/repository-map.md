...
- `utilities/bin/setup-displays.sh` owns the local display layout policy. DDC/CI decides whether the external monitor is powered on; XRandR is only used to apply the selected output layout. A powered-on external monitor is first enabled beside the laptop panel, then it replaces the laptop panel. When no usable external monitor is available, the script restores the laptop panel and turns off stale external outputs. The `50-setup-graphic-tablet-tools` task installs `ddcutil` and loads `i2c-dev` so the power-state check can reach the monitor.
- `utilities/bin/monitor-power-watch` is the user-session trigger for `setup-displays.sh`. It polls external-monitor DDC/CI power state every five seconds, establishes the first sample as a baseline so session startup does not force a layout, and runs `setup-displays.sh` only after a power-state transition. Do not reintroduce udev or KScreen as the primary trigger for monitor power-button changes; HDMI can remain connected while the monitor is powered off.
