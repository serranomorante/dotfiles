...
- System light/dark for terminal AI agents: `utilities/bin/agent-color-mode` writes the Claude Code custom theme and the Codex syntax theme, driven by `utilities/bin/dbus_listen_color_change`; the declarative half (Claude `theme`, Gemini `ui`, OpenCode theme template, Pi telemetry opt-out) lives in `playbooks/roles/20-dev-tools/tasks/170-setup-ai-tools.archlinux.yml`. See `docs/agent-color-mode.md`.
...
- Kitty/tmux: socket/window naming in `term/bin/kitty-window-utils.sh`; tmux copy marks in `term/bin/tmux-copy-mark` and `term/dot-config/tmux/copy-mode-marks.conf`, with the per-session persisted mark files at `$XDG_STATE_HOME/dotfiles/tmux-copy-mark/state-<session-name>`.
- Named marks with custom labels: shared label store in `utilities/bin/marks-common.sh` (sourced library) and `utilities/bin/marks-label` (CLI); labels live per scope at `$XDG_STATE_HOME/dotfiles/marks/<scope>.labels`. tmux copy marks use the session name as scope; Neovim global marks use the cwd key and are wired in `nvim/.../lua/serranomorante/global_marks.lua`. See `docs/named-marks.md`.
