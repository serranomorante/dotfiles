...
- The AI-agent profile does not statically blacklist `~/data/notes/foam` because Firejail blacklists win over whitelists and would hide the notes even from foam-cwd agent runs. Instead `fj-ai-agent` adds `--blacklist ${HOME}/data/notes/foam` only for launches whose work root and extra paths stay outside the foam tree; when the launcher explicitly selects a work root or extra path inside the foam notes tree (e.g. Remind-backed Foam TODO runs whose cwd is the notes root), the wrapper skips the blacklist so the agent can read the notes and the runner writes `misc/agent-runs/` on the host.
...
- `~/dotfiles` is exposed read-write from any project. `~/dotfiles/utilities` and `~/dotfiles/for-my-eyes-only` are `whitelist-ro`; `~/.ssh`, `~/.gnupg`, `~/data/secrets` are blacklisted, and `fj-ai-agent` blacklists `~/data/notes/foam` unless the launch root/extra paths are inside the foam tree.
