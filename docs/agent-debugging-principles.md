...
- Agent tmux sessions use `tmux -L <nvim-server-name>` and place sockets below `/tmp/tmux-$uid`; the AI-agent sandbox keeps the real host `/tmp` (it ignores the dev-tool baseline's `private-tmp`), so the socket directory is shared read-write and the wrapper also exposes it explicitly when present.
...
- Unlike the generic `fj-node`/`fj-py`/`fj-php` wrappers, the AI-agent sandbox does not use `private-tmp`: `/tmp` is the real host `/tmp`, so temp-file handoffs with the user work. Everything under `$HOME` is still hidden except the whitelisted paths.
