...
CPU spike reporting is split into an always-on sampler and a periodic Foam generator. `system-spike-watch.service` runs `system-spike-watch run --interval 5` with low priority; Ansible builds `system-spike-watch` from `utilities/dot-local/share/dotfiles/system-spike-watch/` into `~/bin/` during the Arch dotfiles task. The sampler reads `/proc` every five seconds and only enters short burst capture when an interactive-path process crosses its type-specific threshold. Burst capture samples every 200 ms by default to keep monitor overhead bounded while still catching multi-second freezes. It stores compact JSONL events under `~/.local/state/dotfiles/system-spikes/events/`; it does not write Markdown or query journals while watching.
...
`dotfiles-spikes.timer` runs `dotfiles-spikes update` hourly to turn those JSONL events into `ops/system-health/spikes/system-spikes.md`, source pages, a coverage note, and a monthly digest. Ansible builds `dotfiles-spikes` from `utilities/dot-local/share/dotfiles/dotfiles-spikes/` into `~/bin/`, so report generation avoids Python startup and object overhead in the timer path without doing compilation at runtime. `system-spike-notify.service` is a separate notification layer: it watches the JSONL event directory with `inotifywait` when available and sends clickable `notification-action` notifications for new Xorg events that open the `@id system-spikes-report` block in the periodically refreshed Foam report. Ansible also builds `system-spike-notify` from `utilities/dot-local/share/dotfiles/system-spike-notify/` into `~/bin/`. `dotfiles-health` only reads the current day's event count and links to `[[system-spikes]]`, so the main health report remains cheap.
