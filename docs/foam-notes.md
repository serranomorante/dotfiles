...
Neovim Markdown buffers add a local `@id` metadata convention for paragraph- or list-item-level anchors that should not become headings. An `@id` line must be directly attached to the preceding block, with no blank line between the block's contiguous metadata lines and the `@id`; detached IDs are ignored. Since the Markdown LSP is globally disabled, `gd` resolves links locally: for a wikilink or (non-image) Markdown link under the cursor it opens the target file by direct path, `.md`-suffixed candidate, or unique file-stem search under the workspace root, and jumps to a matching attached `@id` when the fragment is a block target such as `[[note-stem#^decision-sync]]` or `[text](note.md#^decision-sync)`. On save, Neovim warns about duplicate attached `@id` values within the current Markdown file.

Remind-backed TODOs may use `@run agent` inside an indented `remind` fence to run the attached `@id` through the local opencode agent at the scheduled reminder time. The generated Remind `RUN` command goes through the `remind-run` allowlist, queues the agent as a transient user systemd unit so the normal reminder notification is not delayed, resolves the TODO by id, writes the full result under `misc/agent-runs/YYYY-MM/`, logs through the `foam-remind-agent` journal tag, and sends a short completion notification. Unlike interactive agent sessions such as `<leader>cn`, `@run agent` executions always launch unfirejailed (no Firejail `fj-*` wrapper, and Codex runs with `danger-full-access`) and start with the agent's own MCP configuration: each agent is launched directly so it loads the MCP servers declared in its own config files (`~/.claude.json`, `~/.codex/config.toml`, `~/.gemini/settings.json`, or `~/.config/opencode/opencode.jsonc`) instead of pointing at the shared MCPHub config. The Remind agent runner honors explicit `@agent` and `@cwd` metadata lines, but otherwise uses the default opencode agent in the Foam notes root. Repositories named in TODO prose, such as `~/dotfiles`, are read-only context by default: the agent should inspect readable files and leave concrete recommendations, patches, or commands in its agent-run Markdown output instead of editing or committing there.
