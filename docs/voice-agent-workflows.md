...
`record-screen-ffmpeg` records `capture.sink_node.work.monitor` by default, which is intentionally work-output audio only and does not require a static combine-source node. Select `source_node.work_audio_with_mic` for recordings that should also include the RNNoise-processed microphone; that mode mixes the work sink monitor and `source_filter.rnnoise` inside ffmpeg only while the recording is active. The Neovim `record-screen` Overseer task defaults to `source_node.work_audio_with_mic` because interactive screen recordings usually need both Chromium/work audio and the microphone. Use `capture.sink_node.music-production.monitor` for music-production sink recordings.
