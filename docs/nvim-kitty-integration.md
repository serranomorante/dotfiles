...
- `utilities/bin/agent-tasks` is the automation-facing wrapper for sibling agent Overseer tasks; `:AgentTasks` and the related Ex commands are human-facing views over the same module. Assistant sandboxes may block Unix-socket RPC to Neovim and surface that as `E247: Failed to connect ... connection refused`; in that context rerun `agent-tasks` with the required sandbox escalation instead of debugging Lua quoting or replacing the wrapper. A Codex task opened through this bridge may display `continuing this codex conversation with id: ...`; that is normal for a task launched from the current parent conversation and is not by itself evidence that the Overseer child task is invalid or missing metadata.
