...
Wine wrapper tests must be hermetic: never invoke the real `wine`/`wineserver` binaries, even against a temporary `WINEPREFIX`. Real Wine spawns persistent `wineserver` daemons that survive the test, can spike CPU, and interfere with a running workstation prefix such as an open REAPER. Prefer the fake-Wine fixture pattern in `tests/playbooks/wwine_sandbox.sh`: render the wwine template with fake `wine`/`wineserver` executables and assert the wrapper contract through fixture logs (for example the `reg add`/`reg query` arguments). Reserve `# dotfiles-test-firejail: disabled` for Firejail-orchestration and fake-payload tests that intentionally use the real `firejail` binary.
