...
The wrappers use a clean environment and expose only the project root plus explicitly requested paths. Prefer expanding the existing wrappers before creating a new one. The `fj-claude`, `fj-codex`, `fj-gemini`, `fj-opencode`, and `fj-pi` AI-agent wrappers additionally always expose `~/dotfiles` read-write, matching the access agents get when launched with cwd `~/dotfiles`, so agents keep workstation config, scripts, and docs context from any project. AI-agent sandboxes also keep the real host `/tmp` (`ignore private-tmp` on the shared dev-tool baseline) so agents can hand temp files to and from the host; the generic `fj-node`, `fj-py`, and `fj-php` wrappers keep their private `/tmp`.
...
MCP servers generated by the AI tools role are listed in `playbooks/roles/20-dev-tools/templates/mcphub_servers.json`. Use the MCPHub-native `disabled` boolean on each `mcpServers` entry to control whether that server is available in MCPHub; disabled entries stay in the source template for easy re-enablement but are filtered out of generated client-facing configs. Claude and Codex must not receive these MCP servers directly in their default configs: normal `claude` and `codex` launches should stay MCP-free, while `claude-mcp` and `codex-mcp` connect each client to the single local Hub endpoint. Pi has no built-in MCP and ships no MCP launcher in this role; its "no MCP" philosophy routes tools through extensions instead, so `agent-tasks new pi --mcp` reports that Pi has no MCP launcher rather than wrapping it through MCPHub.

The `mcphub.service` user unit is the sole runner: it starts the `mcp-hub` binary on the host so the hub is always reachable, and its managed MCP servers stay sandboxed because the hub spawns each one through `fj-mcp-inherit`, which delegates to `fj-node`/`fj-py` and creates a fresh Firejail sandbox per server. The MCP-aware launchers (`claude-mcp`, `codex-mcp`, `opencode-mcp`) rely on the service being up and connect each client to the single local Hub endpoint. The MCPHub process itself must never run inside an agent sandbox, otherwise `fj-mcp-inherit` takes its inherited-Firejail branch and every backend server dies on the unset `FJ_MCP_INHERITED_PROFILE`.

mcp-hub resolves each `env` value in `mcphub_servers.json` against the hub process environment: an empty-string value is a pass-through reference to a variable of the same name and fails in strict mode with `Variable '<name>' not found` when the hub environment lacks it. The host service environment never defines `FJ_MCP_INHERITED_PROFILE`, so server `env` blocks must not list it at all; leaving the variable unset is what makes `fj-mcp-inherit` take its host branch on spawn.

MCP servers managed by the AI tools role must have telemetry and field-data integrations disabled when upstream supports an opt-out, and servers with unavoidable telemetry should not be exposed through the generated MCP config. Avoid remote HTTP MCP endpoints in the default config unless the endpoint is the explicit product integration being requested and its privacy tradeoff has been reviewed.
