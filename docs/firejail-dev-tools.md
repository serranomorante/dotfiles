...
MCP servers generated by the AI tools role are listed in `playbooks/roles/20-dev-tools/templates/mcphub_servers.json`. Use the MCPHub-native `disabled` boolean on each `mcpServers` entry to control whether that server is available in MCPHub and copied into generated Claude, Gemini, and Codex configs; disabled entries stay in the source template for easy re-enablement but are filtered out of client configs.
