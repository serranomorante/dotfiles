...
Ctrl+Alt+P opens the shared snippets picker through `signal_open_snippets`; the picker reads the public `utilities/dot-local/share/dotfiles/snippets/` tree and, when present, the optional private `for-my-eyes-only/dot-local/share/dotfiles/snippets/` tree. The same `snippets` script also supports direct paste from Neovim: `<leader>sP` runs `snippets <name> var:KEY=VALUE ...`, resolving the snippet by basename, stripping metadata, substituting `{var:KEY}` placeholders (for example `var:AGENT_NAME=...`, `var:CWD=...`, `var:CHAT_ID=...` filled from the current agent session), and reusing the same clipboard/terminal paste handoff. Snippet bodies can also embed `{regex:s/pat/rep/flags}` placeholders that expand to the current clipboard text run through that sed-style substitution: each clipboard line is processed independently, `\1`..`\9` backreferences are rewritten to `$1`..`$9`, and Perl escapes/flags (`gimsx`) are available; the fzf preview pane resolves the marker against the live clipboard so you see the converted result before choosing, and a paste with an unreadable clipboard aborts before anything is copied. Ctrl+Alt+B opens the Pedalboard MIDI profile picker through `signal_open_pedalboard_profiles`; profile metadata is owned by `audio/dot-local/share/dotfiles/pedalboard-midi-profiles.tsv`.
