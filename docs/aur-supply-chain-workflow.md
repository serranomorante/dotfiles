...
When `aur-review publish` fails at source verification because the AUR PKGBUILD records a checksum that no longer matches the upstream forge archive, do not patch the checksum inside the review checkout: `aur-review` rejects any local worktree change beyond the generated VCS `pkgver=` update. Confirm the archive's current SHA-256 in a throwaway clone, flag the AUR package as out-of-date, and wait for the maintainer to fix the PKGBUILD before re-running `accept` and `publish`; the package can stay on its Ansible `aur_local_packages` list meanwhile, since the install task reports it as unpublished and skips it.
