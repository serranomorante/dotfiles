...
`aur-review publish` refuses to run when `origin/master` has unreviewed commits, exports the accepted `master` commit to an isolated temporary build tree, verifies sources with `makepkg --verifysource`, builds with `makechrootpkg` in a clean chroot by default, copies the resulting `.pkg.tar*` files into `/var/cache/pacman/aur-local`, and updates the `[aur-local]` package database with `repo-add`. The isolated build tree prevents VCS `pkgver()` updates or other `makepkg` side effects from dirtying the reviewed checkout under `~/data/aur-review`. Clean-chroot builds can resolve already-published local AUR dependencies from `[aur-local]`, but those dependencies must be reviewed and published first; for example, publish `python-sabctools` before publishing `sabnzbd`.

When an AUR PKGBUILD cannot build as-is because upstream metadata is stale, `aur-review publish` applies repo-owned patches inside the isolated build tree, so the review checkout under `~/data/aur-review` stays clean. Patches live at `assets/patches/aur-review/<pkg>/*.patch` and are applied in sorted order before `makepkg --verifysource` and the build, with `patch --fuzz=0` so a patch only applies while every line it references is unchanged; a patch that no longer applies cleanly fails publish with a message to regenerate or remove it. This handles cases like a PKGBUILD `sha256sums` that no longer matches the upstream forge archive (for example the rtcqs checksum fix), and a routine AUR `pkgrel` bump leaves the patch applicable without any rename. Keep such patches minimal and review them in the dotfiles diff like any source change; still flag the package as out-of-date so the maintainer fixes the PKGBUILD and the patch can be removed. A package on its Ansible `aur_local_packages` list stays there meanwhile, since the install task reports it as unpublished and skips it until `publish` succeeds.

If an accepted AUR package has signed sources, keep the required `makepkg` PGP public keys in `arch_aur_makepkg_pgp_keys` under the `10-system-tools` defaults. The `10-20` AUR setup tag imports those pinned full fingerprints into the user's GPG keyring before `aur-review publish` needs `makepkg --verifysource`; do not patch around signature verification with `--skippgpcheck`.
