...
keyboard-midi-controller-dotfiles-contract)
    ...
    rg -q 'desktop-action-state SLOT LABEL VALUE' "${source_dir}/main.go"
    rg -q 'desktopActionSlotCount = 5' "${source_dir}/main.go"
    rg -q '\[desktopActionSlotCount\]string' "${source_dir}/main.go"
    rg -q 'slot must be 0-%d' "${source_dir}/main.go"
    rg -q 'parsePedalboardState' "${source_dir}/main.go"
    rg -q 'setPedalboardState' "${source_dir}/main.go"
    rg -q 'parseDesktopActionState' "${source_dir}/main.go"
    rg -q 'setDesktopActionState' "${source_dir}/main.go"
    rg -q 'sanitizeTFTToken' "${source_dir}/main.go"
    ...
    rg -q 'setDesktopAction\(slot int, label string, value string\)' "${source_dir}/main.go"
    ...
    rg -q 'desktop-action-state SLOT LABEL VALUE' "$wrapper"
    ...
    rg -q 'keyd-.*-midi-layers-v\d+' "$keyboard_task"
    rg -q '    - keyboard-midi-controller.service$' "$keyboard_task"
    rg -q '"\[\{\{ user_os \}\}\] Keyboard peripheral: compile keyboard MIDI controller"' "$keyboard_task"
    rg -q '      - "\{\{ ansible_facts\.env\.HOME \}\}/bin/keyboard-midi-controller"$' "$keyboard_task"
    rg -q '      - --compile-cache$' "$keyboard_task"
    rg -q '"\[\{\{ user_os \}\}\] Keyboard peripheral: check running keyboard MIDI controller cache"' "$keyboard_task"
    ...
    rg -q '"\[\{\{ user_os \}\}\] Keyboard peripheral: restart keyboard MIDI controller after cache refresh"' "$keyboard_task"
    ...
    rg -q '"\[\{\{ user_os \}\}\] Keyboard peripheral: stop retired cursor indicator services"' "$keyboard_task"
    ...
    rg -q '"\[\{\{ user_os \}\}\] Keyboard peripheral: remove retired cursor indicator files"' "$keyboard_task"
    ...
    rg -q '"\[\{\{ user_os \}\}\] Keyboard peripheral: stat keyd observer script"' "$keyboard_task"
    rg -q 'path: ~/dotfiles/peripherals/bin/keyd-observer' "$keyboard_task"
    rg -q '"\[\{\{ user_os \}\}\] Keyboard peripheral: record applied keyd observer checksum"' "$keyboard_task"
    rg -q 'keyd-observer-sha256' "$keyboard_task"
    rg -q 'handler_restart_keyd_observer_service' "$keyboard_task"
    rg -q '"\[\{\{ user_os \}\}\] Keyboard peripheral: prepare readline-mode-notify script"' "$keyboard_task"
    ...
    rg -q 'CONTINUOUS_DEFAULT_ZERO_PERCENT = 15' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_DEFAULT_FULL_PERCENT = 60' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_MIN_RANGE_PERCENT = 5' "$pedalboard_firmware"
    rg -q 'RANGE_MAGIC_ADDR = 2' "$pedalboard_firmware"
    rg -q 'RANGE_ZERO_ADDR = 3' "$pedalboard_firmware"
    rg -q 'RANGE_FULL_ADDR = 4' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_SAMPLE_COUNT = 8' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_EMIT_DEADBAND = 2' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_REVERSAL_THRESHOLD = 6' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_REVERSAL_CONFIRMATIONS = 3' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_MAX_STEP = 4' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_ENDPOINT_DEADBAND = 2' "$pedalboard_firmware"
    rg -q '^int continuousRawValue\(\) \{$' "$pedalboard_firmware"
    rg -q '^int rawForPercent\(uint8_t percent\) \{$' "$pedalboard_firmware"
    rg -q '^int continuousMappedValue\(\) \{$' "$pedalboard_firmware"
    rg -q 'raw <= zeroRaw' "$pedalboard_firmware"
    rg -q 'raw >= fullRaw' "$pedalboard_firmware"
    rg -q 'map\(raw, zeroRaw, fullRaw, 0, 127\)' "$pedalboard_firmware"
    rg -q '^int stepContinuousToward\(int current, int target\) \{$' "$pedalboard_firmware"
    rg -q '^int continuousValue\(\) \{$' "$pedalboard_firmware"
    rg -q 'value == 0 \|\| value == 127' "$pedalboard_firmware"
    rg -q 'continuousDirection = 0;' "$pedalboard_firmware"
    rg -q 'continuousReversalCandidate' "$pedalboard_firmware"
    rg -q 'continuousReversalCount >= CONTINUOUS_REVERSAL_CONFIRMATIONS' "$pedalboard_firmware"
    ...
    rg -q '^bool validContinuousRange\(uint8_t zeroPercent, uint8_t fullPercent\) \{$' "$pedalboard_firmware"
    rg -q '^void saveContinuousRange\(\) \{$' "$pedalboard_firmware"
    rg -q '^void loadContinuousRange\(\) \{$' "$pedalboard_firmware"
    rg -q 'range <zero_percent> <full_percent>' "$pedalboard_firmware"
    rg -q 'range reset' "$pedalboard_firmware"
    rg -q 'range_zero=' "$pedalboard_firmware"
    rg -q 'range_full=' "$pedalboard_firmware"
    ...
    rg -q '\{"obs-mouseless-setup", 14, 4, 80, 81, SWITCH_MOMENTARY, SWITCH_MOMENTARY\}' "$pedalboard_firmware"
    rg -q 'profile <piano\|guitar\|desktop\|obs-mouseless-setup>' "$pedalboard_firmware"
    ...
    rg -q 'sscanf\(line, "D %d %8s %8s"' "$tft_firmware"
    rg -q '^const uint8_t CC_BASE = 10;$' "$tft_firmware"
    rg -q '^const uint8_t PEDALBOARD_CHANNEL = 15;$' "$tft_firmware"
    rg -q '^const uint8_t DESKTOP_ACTION_SLOT_COUNT = 5;$' "$tft_firmware"
    rg -q 'Ch 15 Pedalboard' "$tft_firmware"
    rg -q 'pedalboardProfileLabel' "$tft_firmware"
    rg -q 'tileKeyLabel' "$tft_firmware"
    rg -q 'pedalboardModifierTile' "$tft_firmware"
    rg -q 'pedalboardModifierActive' "$tft_firmware"
    rg -q 'bool modifierDotDirty\[2\];' "$tft_firmware"
    rg -q 'markPedalboardModifierDotsDirty' "$tft_firmware"
    rg -q 'drawPedalboardModifierDot\(uint8_t col\)' "$tft_firmware"
    rg -q 'strcmp\(desktopActionLabels\[0\], "MODE"\) == 0 && strcmp\(desktopActionValues\[0\], "SHIFT"\) == 0' "$tft_firmware"
    rg -q 'modifierColor = pedalboardModifierActive\(\) \? ILI9341_YELLOW : ILI9341_DARKGREY' "$tft_firmware"
    rg -q 'fillCircle\(x \+ w - 6, y \+ 6, 3, modifierColor\)' "$tft_firmware"
    refute rg -q 'fillCircle\(x \+ w - 6, y \+ 6, 3, ILI9341_YELLOW\)' "$tft_firmware"
    rg -q 'colorForPedalboard' "$tft_firmware"
    rg -q 'desktopActionKnown' "$tft_firmware"
    rg -q 'desktopActionValueActive' "$tft_firmware"
    rg -q 'pedalboardDesktopActionTile\(uint8_t row, uint8_t col\)' "$tft_firmware"
    rg -q 'return col < 5' "$tft_firmware"
    rg -q 'pedalboardProfile == 4 && (col == 1 || col == 2)' "$tft_firmware"
    rg -q 'case 4:' "$tft_firmware"
    rg -q 'return "obs"' "$tft_firmware"
    rg -q 'constrain\(first, 0, 4\)' "$tft_firmware"
    rg -q '\{"VEL", "SCENE", "LINE", "", ""\}' "$tft_firmware"
    rg -q 'ILI9341_DARKGREY, ILI9341_RED, ILI9341_CYAN, ILI9341_DARKGREY, ILI9341_DARKGREY' "$tft_firmware"
    refute rg -q 'pedalboardProfile == 3 && col > 0 && desktopActionKnown\(col\)' "$tft_firmware"
    rg -q 'desktopActionFlashUntil' "$tft_firmware"
    rg -q 'desktopActionFlashValue' "$tft_firmware"
    rg -q 'strcmp\(value, "READY"\) != 0' "$tft_firmware"
    rg -q 'strcmp\(value, "IDLE"\) != 0' "$tft_firmware"
    rg -q 'strcmp\(value, "UNKNOWN"\) != 0' "$tft_firmware"
    rg -q 'strcmp\(value, "BASE"\) != 0' "$tft_firmware"
    rg -q 'strcmp\(value, "SAVED"\) == 0 \|\| strcmp\(value, "ERROR"\) == 0' "$tft_firmware"
    rg -q 'snprintf\(desktopActionValues\[slot\], sizeof\(desktopActionValues\[slot\]\), "READY"\)' "$tft_firmware"
    rg -q '\{"DAMP", "SOST", "SOFT", "SOST", "SOFT"\}' "$tft_firmware"
    rg -q '\{"EXP", "STMPA", "STMPB", "STMPA", "STMPB"\}' "$tft_firmware"
    rg -q '\{"CTRL", "ACT A", "ACT B", "ACT A", "ACT B"\}' "$tft_firmware"
    refute rg -q '"PROF"' "$tft_firmware"
    rg -q 'pedalboardContinuous != nextContinuous' "$tft_firmware"
    rg -q 'tileDirty\[0\]\[0\] = true;' "$tft_firmware"
    rg -q 'pedalboardSwitch1 != nextSwitch1' "$tft_firmware"
    rg -q 'tileDirty\[0\]\[1\] = true;' "$tft_firmware"
    rg -q 'pedalboardSwitch2 != nextSwitch2' "$tft_firmware"
    rg -q 'tileDirty\[0\]\[2\] = true;' "$tft_firmware"
    rg -q 'first < DESKTOP_ACTION_SLOT_COUNT' "$tft_firmware"
    rg -q 'tileDirty\[0\]\[slot\] = true;' "$tft_firmware"
    rg -q 'if \(slot == 0\)' "$tft_firmware"
    rg -q 'markPedalboardModifierDotsDirty\(\);' "$tft_firmware"
    refute rg -q 'tileDirty\[0\]\[3\] = true;' "$tft_firmware"
    refute rg -q 'tileDirty\[0\]\[4\] = true;' "$tft_firmware"
    ...
    rg -q 'exec_timeout_no_wait = -1' "$item_state_feedback"
    rg -q 'exec_timeout_no_wait = -1' "$midi_editor_state_feedback"
    rg -q 'exec_timeout_no_wait = -1' "$project_transport_state_feedback"
    refute rg -q 'command_timeout_ms' "$item_state_feedback"
    refute rg -q 'command_timeout_ms' "$midi_editor_state_feedback"
    refute rg -q 'command_timeout_ms' "$project_transport_state_feedback"
    refute rg -q '</dev/null' "$item_state_feedback"
    refute rg -q '</dev/null' "$midi_editor_state_feedback"
    refute rg -q '</dev/null' "$project_transport_state_feedback"
