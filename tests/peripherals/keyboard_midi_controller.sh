...
pedalboard_firmware="${DOTFILES_TEST_ROOT}/assets/firmware/pedalboard-midi-controller/pedalboard-midi-controller.ino"
...
vim-register-clipboard-keyd-template-contract)
    ...
    refute rg -q 'signal_paste_vim_register' "$keyd_template"
    refute rg -q 'signal_paste_vim_register|paste_vim_register|vim_register_clipboard' "$keyd_observer"
    ;;
keyboard-midi-controller-dotfiles-contract)
    refute rg -q '^\[signal_toggle_keyboard_midi_controller\]$' "$keyd_template"
    ...
    refute rg -q '^9 = layer\(midi_bank_down\)$' "$keyd_template"
    refute rg -q '^0 = layer\(midi_bank_up\)$' "$keyd_template"
    refute rg -q '^o = layer\(midi_channel_down\)$' "$keyd_template"
    refute rg -q '^p = layer\(midi_channel_up\)$' "$keyd_template"
    refute rg -q '^0 = layer\(midi_enc_10\)$' "$keyd_template"
    ...
    rg -q 'disableLEDEnv\s*=\s*"KMC_DISABLE_LED"' "${source_dir}/main.go"
    rg -q 'tftSerialPortEnv\s*=\s*"KMC_TFT_SERIAL_PORT"' "${source_dir}/main.go"
    rg -q 'runLEDAutoConnectLoop' "${source_dir}/main.go"
    rg -q 'envFlagEnabled\(disableLEDEnv\)' "${source_dir}/main.go"
    rg -q '"led_disabled":\s*envFlagEnabled\(disableLEDEnv\)' "${source_dir}/main.go"
    ...
    rg -q 'pedalboard-state PROFILE CONT SW1 SW2' "${source_dir}/main.go"
    rg -q 'parsePedalboardState' "${source_dir}/main.go"
    rg -q 'setPedalboardState' "${source_dir}/main.go"
    rg -q 'pedalboardProfileID' "${source_dir}/main.go"
    rg -q 'setPedalboard\(profile int, continuous int, switch1 int, switch2 int\)' "${source_dir}/main.go"
    ...
    rg -q 'pedalboard-state PROFILE CONT SW1 SW2' "$wrapper"
    ...
    refute rg -q 'SND_SEQ_PORT_TYPE_APPLICATION' "${source_dir}/main.go"
    ...
    refute rg -q 'exclusiveInputGrab|eviocgrab|EVIOCGRAB|targetInputName|ioctlGrab|keyd virtual keyboard|handleInputEvent|inputDevice|inputEvent' "${source_dir}/main.go"
    refute rg -q 'cursor_indicator|midiIndicatorColor' "${source_dir}/main.go"
    refute rg -q 'leaveMouselessMode' "${source_dir}/main.go"
    ...
    rg -q 'keyd-.*-midi-layers-v1' "$keyboard_task"
    ...
    refute rg -q 'clone cursor indicator|make cursor indicator|symlink cursor indicator' "$keyboard_task"
    ...
    rg -q 'keyd-observer-sha256' "$keyboard_task"
    ...
    refute rg -q 'Dotfiles: record applied keyd observer checksum' "$dotfiles_task"
    ...
    refute rg -q 'keyd reload|handler_restart_keyd_result' "${DOTFILES_TEST_TMP}/keyd-restart-handler"
    ...
    rg -q '^Environment="KMC_DISABLE_LED=1"$' "$unit_file"
    ...
    [[ -s "$pedalboard_firmware" ]]
    rg -q '^#include "MIDIUSB.h"$' "$pedalboard_firmware"
    rg -q '^const uint8_t SWITCH_1_PIN = 2;$' "$pedalboard_firmware"
    rg -q '^const uint8_t SWITCH_2_PIN = 3;$' "$pedalboard_firmware"
    rg -q '^const uint8_t CONTINUOUS_PIN = A0;$' "$pedalboard_firmware"
    rg -q '^const bool SWITCH_INPUT_INVERTED = true;$' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_RAW_MIN = 30' "$pedalboard_firmware"
    rg -q 'CONTINUOUS_RAW_MAX = 480' "$pedalboard_firmware"
    rg -q '^bool switchPressed\(uint8_t pin\) \{$' "$pedalboard_firmware"
    rg -q 'return SWITCH_INPUT_INVERTED \? !pulledLow : pulledLow;' "$pedalboard_firmware"
    rg -q 'bool rawPressed = switchPressed\(state\.pin\);' "$pedalboard_firmware"
    rg -q 'bool switch1Pressed = switchPressed\(SWITCH_1_PIN\);' "$pedalboard_firmware"
    rg -q 'bool switch2Pressed = switchPressed\(SWITCH_2_PIN\);' "$pedalboard_firmware"
    rg -q 'sendControlChange\(profile\.channel, controller, pressed \? 127 : 0\);' "$pedalboard_firmware"
    rg -q '\{"piano", 0, 64, 66, 67, SWITCH_MOMENTARY, SWITCH_MOMENTARY\}' "$pedalboard_firmware"
    rg -q '\{"guitar", 1, 4, 80, 81, SWITCH_LATCH, SWITCH_LATCH\}' "$pedalboard_firmware"
    rg -q '\{"desktop", 15, 4, 80, 81, SWITCH_MOMENTARY, SWITCH_MOMENTARY\}' "$pedalboard_firmware"
    rg -q 'profile <piano\|guitar\|desktop>' "$pedalboard_firmware"
    rg -q 'applyBootProfileOverride' "$pedalboard_firmware"
    [[ -s "$tft_firmware" ]]
    ...
    rg -q 'sscanf\(line, "B %d %d %d %d"' "$tft_firmware"
    rg -q '^const uint8_t CC_BASE = 10;$' "$tft_firmware"
    rg -q '^const uint8_t PEDALBOARD_CHANNEL = 15;$' "$tft_firmware"
    rg -q 'Ch 15 Pedalboard' "$tft_firmware"
    rg -q 'pedalboardProfileLabel' "$tft_firmware"
    rg -q 'colorForPedalboard' "$tft_firmware"
    rg -q '\{"PROF", "DAMP", "SW1", "SW2"\}' "$tft_firmware"
    ...
    refute rg -q 'one_lane = 45' "$item_state_feedback"
    refute rg -q 'C_LANEPLAYS:%d' "$item_state_feedback"
    ...
    rg -q 'record = 39' "$project_transport_state_feedback"
    ...
    rg -q 'GetPlayState' "$project_transport_state_feedback"
    rg -q 'has_flag\(play_state, 4\)' "$project_transport_state_feedback"
...
PY
    ...
    rg -q 'keyboard-midi-led-matrix-firmware-sha256' "$embedded_task"
    rg -q 'keyboard-midi-tft-display/keyboard-midi-tft-display\.ino' "$embedded_task"
    rg -q 'keyboard-midi-tft-display-firmware-sha256' "$embedded_task"
    rg -q 'pedalboard-midi-controller/pedalboard-midi-controller\.ino' "$embedded_task"
    rg -q 'dotfiles_upload_pedalboard_midi_controller' "$embedded_task"
    ...
    refute rg -q '/usr/bin/rg|echo "\$LINE"|while read LINE' "$readline_notify_watcher"
    ...
    refute rg -q 'org.freedesktop.Notifications|Notify|CloseNotification|redis-cli|notification_id|assets/icons/osd|input-mouse|input-keyboard|audio-card' "$show_mouseless_osd" "$hide_mouseless_osd" "$show_readline_osd" "$hide_readline_osd" "$show_keyboard_midi_osd" "$hide_keyboard_midi_osd"
    refute rg -q '^\[(mouseless_mode|readline_mode|keyboard_midi_mode)\]$|show_mouseless_osd|show_readline_osd|show_keyboard_midi_osd|assets/icons/osd|input-mouse|input-keyboard|audio-card' "$dunst_config"
    refute rg -q 'cursor_indicator' "$show_mouseless_osd" "$hide_mouseless_osd" "$show_readline_osd" "$hide_readline_osd" "$show_keyboard_midi_osd" "$hide_keyboard_midi_osd"
