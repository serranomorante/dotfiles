...
plasma-dwm-playbook-masks-kwin-and-enables-dwm)
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: remove packages" "name: kwin"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: remove packages" "state: absent"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: remove packages" "force: true"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: ensure mask kwin service" "name: plasma-kwin_x11.service"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: ensure mask kwin service" "enabled: false"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: ensure mask kwin service" "masked: true"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: kwriteconfig6" "kwriteconfig6 --file startkderc --group General --key systemdBoot true"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: ensure dwm service" "name: plasma-wm.service"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: ensure dwm service" "enabled: true"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: ensure dwm service" "masked: false"
    ;;
plasma-dwm-patched-checkout-is-marker-gated)
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: stat dwm patch marker" "dotfiles_marker_name"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: clone latest dwm" "tasks_from: git"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: clone latest dwm" "update_diff_git_force: true"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: clone latest dwm" "when: not var_dwm_patch_marker.stat.exists"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: apply dwm patches" "when: not var_dwm_patch_marker.stat.exists"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: record dwm patch marker" "when: not var_dwm_patch_marker.stat.exists"
    assert_task_contains "$compositor_tasks" "[{{ user_os }}] Compositor: install dwm" "not var_dwm_patch_marker.stat.exists or not var_installed_dwm.stat.exists"
    ;;
...
plasma-dwm-autostart-policy-keeps-plasma-target-loadable)
    ...
    assert_task_contains "$autostart_tasks" "[{{ user_os }}] XDG autostart policy: mask systemd XDG autostart generator" "dest: /etc/systemd/user-generators/systemd-xdg-autostart-generator"
    assert_task_contains "$autostart_tasks" "[{{ user_os }}] XDG autostart policy: mask systemd XDG autostart generator" "src: /dev/null"
    assert_task_contains "$autostart_tasks" "[{{ user_os }}] XDG autostart policy: remove stale user target mask" "path: ~/.config/systemd/user/xdg-desktop-autostart.target"
    assert_task_contains "$autostart_tasks" "[{{ user_os }}] XDG autostart policy: remove stale user target mask" "state: absent"
    ;;
