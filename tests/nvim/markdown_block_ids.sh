...
run_nvim_lua() {
    local lua=$1
    local harness='local function dotfiles_test_later(ms, fn) vim.defer_fn(function() local ok, err = xpcall(fn, debug.traceback); if not ok then print(err); vim.cmd("cquit 1"); end end, ms) end; _G.dotfiles_test_later = dotfiles_test_later; local ok, err = xpcall(function() '
    local trailer=' end, debug.traceback); if not ok then print(err); vim.cmd("cquit 1"); end'
    "$nvim_bin" --headless -u NONE -c "set rtp^=${rtp}" -c "lua ${harness}${lua}${trailer}"
}
