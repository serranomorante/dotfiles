local function separator(sel) return table.concat({ sel and "%#TabLineSepSel#" or "%#TabLineSep#", "" }) end
...
local function title(tabpage, bufnr, sel)
  ...
  if sel then return table.concat({ "%#TabLineSel#", "%=", (" %s "):format(text) }) end
...
local function modified(bufnr, sel)
  ...
  if not vim.api.nvim_get_option_value("modified", { buf = bufnr }) then return "" end
...
vim.go.tabline = [[%{%v:lua.user.draw()%}]]
