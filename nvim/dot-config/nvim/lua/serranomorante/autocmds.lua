...
vim.api.nvim_create_autocmd("FileChangedShellPost", {
  ...
  callback = function(args)
    ...
    if not require("serranomorante.plugins.lsp.utils").should_enable_marksman(bufnr) then return end

    for _, client in ipairs(vim.lsp.get_clients({ name = "marksman", bufnr = bufnr, _uninitialized = true })) do
      client:stop(true)
    end
    require("serranomorante.plugins.lsp.utils").enable("marksman", bufnr)
  end,
})
